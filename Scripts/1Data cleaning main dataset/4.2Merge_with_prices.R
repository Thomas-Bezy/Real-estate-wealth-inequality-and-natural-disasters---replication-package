...
fideli=read_dta("Data/info_for_dvf2017.dta")
dvf=read_dta("Data/dvf_total.dta")
insee_postal=read_dta("Data/correspondance-code-insee-code-postal.dta")
...
for (t in 1:300){
  ...
  for (i in (thresholds[t]+1):thresholds[(t+1)]) {
    ...
    if (i %in% thresholds[2:301]){
      ...
      write_dta(merged,paste0("Data/dvf_merged2017/df_merged_part",t,".dta"))
      write_dta(multiple,paste0("Data/dvf_merged2017/df_multiple_part",t,".dta"))
      write_dta(to_merge,paste0("Data/dvf_merged2017/df_tomerge_part",t,".dta"))
    }else{}
...
df_final=data.frame()
for (t in 1:300) {
  temp=read_dta(paste0("Data/dvf_merged2017/df_merged_part",t,".dta"))
  df_final=rbind(df_final,temp)
  print(t)
}
df_final=df_final%>%select(id_log,PRICE,STAGE,SURF)

write_dta(df_final,"Data/id_log_prices2017.dta")
