...
for (k in 1:3){
  ...
  for (i in data_list){
    ...
    temp=data_frame(read_sf(paste("Data/Boundaries_separate_files/TRI_fideli_w_exposure/_fideli_w_exposure",k,"_",i,".gpkg",sep="")))%>%
...
#Export data
write_dta(tri,"Data/_fideli_w_tri_without_dem.dta")
...
for (k in 1:3){
  ...
  for (i in c(1:1200)){
    ...
    temp=data_frame(read_sf(paste("Data/Boundaries_separate_files/RGA_fideli_w_exposure/_fideli_w_exposure",k,"_",i,".gpkg",sep="")))%>%
      select(-geom,-fid_2)
    if (nrow(temp)>0){
      rga=rbind(rga,temp)
    }else{}
...
write_dta(rga,"Data/_fideli_w_rga_without_dem1.dta")
...
write_dta(rga,"Data/_fideli_w_rga_without_dem2.dta")
...
write_dta(rga,"Data/_fideli_w_rga_without_dem3.dta")
