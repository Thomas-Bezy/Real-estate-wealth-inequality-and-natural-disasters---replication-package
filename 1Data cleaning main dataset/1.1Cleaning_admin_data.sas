...
libname baseline "\\casd.fr\casdfs\Projets\ESSCODI\Data\Logement_FIDELI_2017";
...
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\fideli_loc_1.csv"
replace;
delimiter=",";
run;
...
proc export data=raw_data.fideli_loc_metro DBMS=csv
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\fideli_loc_2.csv"
replace;
delimiter=",";
run;
...
proc export data=raw_data.fideli_loc_metro DBMS=csv
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\fideli_loc_3.csv"
replace;
delimiter=",";
run;

proc export data=raw_data.fideli_loc_om DBMS=csv
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\fideli_loc_4.csv"
replace;
delimiter=",";
run;

proc export data=raw_data.fideli_loc_om (keep= id_log x y) DBMS=dta
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\_fideli_nodata_4.dta"
replace;
run;
...
*proc export data=raw_data.fideli_sec DBMS=dta
*outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\_fideli_sec.dta"
*replace;
*run;
...
*We take out all duplicated rows by id_log, so that we exclude all individuals in the same 
households but keep owners who are in different households;
/*
data raw_data.fideli_sec_char;
set raw_data.fideli_sec_char;
by id_log notsorted;
retain count;
if First.id_log  then count=0;
else count+1;
run;

data raw_data.fideli_sec_char (drop=count);
set raw_data.fideli_sec_char;
where count=0;
run;
*/
...
by id_log_sec;
run;
*A secondary home can be owned by max 2 different households;
*It is very rare: it is only for 26 000 secondary homes;
*We just ignore the problem: 26 430/2 360 000 do not matter;
...
*36 626 008 dwellings that we cover;
*After the merge we have 36 692 659 dwellings;

*Why do we see this?;
*Explanation 1: some secondary homes have multiple owners (weight<1). This is normal.;
*Explanation 2: some secondary homes are actually primary homes for others.
As stated in Fideli, we should consider these homes as primary homes.
Thus, we exclude the info we have on secondary home ownership.;
...
where count=1;
run;
*36 572 835 observations if we remove all observations that appear at least twice;
...
*Double check that we have the same number of observations than at the beginning;
*5 000 en plus... Don't know why;
proc sort data=raw_data.fideli_dem2 out=raw_data.temp nodupkey;
by id_log;
run;
...
*We export the data;
proc export data=raw_data.fideli_dem2 DBMS=dta
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\_fideli_dem.dta"
replace;
run;
