...
*****ADD CODNAIS, mobility and characteristics of last building;


******For Fideli_diff_1;

data raw_data.fideli_ind (keep= id_log codnais type_menf id_log2016);
set baseline.fideli_individu17_diff_1 ;
run;
...
data raw_data.fideli_ind_append (keep= id_log codnais type_menf id_log2016);
set baseline.fideli_individu17_diff_2 ;
run;

proc append base=raw_data.fideli_ind data=raw_data.fideli_ind_append;
run;
...
****Change IDs for id_log and id_log_n_1;


data raw_data.fideli_ind_n1 (drop=codnais type_menf) ;
set raw_data.fideli_ind ;
rename id_log=ID;
rename id_log2016=ID_N1;
run;

data raw_data.fideli_ind_n1 ;
set raw_data.fideli_ind_n1 ;
where ID not=.;
run;

*We export the data;
proc export data=raw_data.fideli_ind_n1 DBMS=dta
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\fideli_ind2022_n1.dta"
replace;
run;
...
****Merge for owners;
data raw_data.codnais (drop=codnais ID_N1) ;
set raw_data.codnais ;
flag=1;
rename ID=ID_OWNER;
where ID not=.;
...
*We export the data;
proc export data=raw_data.fideli_dem3 DBMS=dta
outfile="C:\Users\Public\Documents\Thomas\Insurance\0Data\codnais2017.dta"
replace;
run;
