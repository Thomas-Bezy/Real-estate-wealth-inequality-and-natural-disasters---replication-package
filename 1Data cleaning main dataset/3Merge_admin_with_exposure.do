...
**********TRI**********
use "$dir\_fideli_w_tri_without_dem.dta", replace

**********Subsidence**********
merge n:1 id_log using "$dir\_fideli_w_rga_without_dem.dta", generate(merge)
drop if merge==2
drop merge

**********Merge with demographics**********
merge 1:n ID using "$dir\_fideli_dem.dta", generate(merge)
drop merge
...
*Merge with Zones d'Emploi (=commuting zones)

rename MUNICIPALITY CODGEO

merge n:1 CODGEO using "$dir\zones_emplois.dta", generate(merge)
drop if merge==2
rename ZE2020 ZE
drop merge
